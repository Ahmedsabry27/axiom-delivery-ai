...
export function useAgentMutations(){const queryClient=useQueryClient();const refresh=(_data,variables)=>{queryClient.invalidateQueries({queryKey:["agents"]});if(variables?.id)queryClient.invalidateQueries({queryKey:["agent",variables.id]});};return {create:useMutation({mutationFn:createAgent,onSuccess:refresh}),update:useMutation({mutationFn:({id,payload,lockVersion})=>updateAgent(id,payload,lockVersion),onSuccess:refresh}),lifecycle:useMutation({mutationFn:({id,action,lockVersion,payload})=>lifecycleAgent(id,action,lockVersion,payload),onSuccess:refresh})};}
