...
export default function CopilotInspector({contextId,messages,runtime,onContextChange}){
 ...
 {tab==="Activity"&&<section><h2 className="font-display flex items-center gap-2 text-xl font-bold"><Radio size={18}/>Safe activity</h2><div className="mt-4 space-y-2">{activity.length?activity.map((item,index)=><div key={item.id||index} className="border-l-4 border-stone-300 bg-[#faf8f5] p-3"><strong className="text-sm">{item.name||item.type||"Runtime event"}</strong><p className="mt-1 text-xs text-stone-500">{item.status||"Recorded"}</p></div>):<p className="text-sm text-stone-500">No runtime activity recorded yet.</p>}</div><p className="mt-4 text-xs text-stone-500">Private reasoning, raw prompts, credentials and restricted evidence are never shown.</p></section>}
 </div></aside>;
}
