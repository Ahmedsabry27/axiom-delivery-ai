...
SPRINTS = (
    (
        "KPI A",
        "Deliver a measurable integrated foundation",
        "2026-07-13T08:00:00.000Z",
        "2026-07-26T17:00:00.000Z",
    ),
    (
        "KPI B",
        "Complete controls and release readiness",
...
def require(response: httpx.Response, operation: str, accepted=(200, 201, 204)) -> dict:
    if response.status_code not in accepted:
        try:
            ...
            errors = []
        raise RuntimeError(
            f"{operation} failed ({response.status_code}): {'; '.join(map(str, errors)) or 'Provider rejected request'}"
        )
...
def transition(client: httpx.Client, issue_key: str, target: str) -> bool:
    issue = require(
        client.get(f"/rest/api/3/issue/{issue_key}", params={"fields": "status"}),
        f"Read status {issue_key}",
    )
    ...
    transitions = require(
        ...
        f"Read transitions {issue_key}",
    ).get("transitions", [])
    selected = next(
        (
            row
            for row in transitions
            if row.get("to", {}).get("statusCategory", {}).get("key") == target
        ),
        None,
    )
    if not selected and target == "done":
        ...
        transitions = require(
            client.get(f"/rest/api/3/issue/{issue_key}/transitions"),
            f"Reload transitions {issue_key}",
        ).get("transitions", [])
        selected = next(
            (
                row
    ...
    require(
        client.post(
            f"/rest/api/3/issue/{issue_key}/transitions",
            json={"transition": {"id": selected["id"]}},
        ),
    ...
    try:
        ...
        with httpx.Client(
            ...
            for project in projects:
                ...
                if project["key"] in LEGACY_KEYS or (
                    prop.status_code == 200
                    and prop.json().get("value", {}).get("entityType") == "PROJECT"
                ):
                    selected.append(project)
            link_types = require(
                client.get("/rest/api/3/issueLinkType"), "Link type lookup"
            ).get("issueLinkTypes", [])
            blocks_type = next(
                (
                    row["name"]
                    for row in link_types
                    if row.get("name", "").lower() == "blocks"
                ),
                None,
            )
            for project in selected:
                key = project["key"]
                detail = require(
                    client.get(f"/rest/api/3/project/{key}"), f"Project {key}"
                )
                issue_types = {
                    row["name"].lower(): row["id"]
                    for row in detail.get("issueTypes", [])
                }
                ...
                versions = require(
                    client.get(f"/rest/api/3/project/{key}/versions"), f"Versions {key}"
                )
                version_name = f"{key} Outcome Release 1.0"
                version = next(
                    (row for row in versions if row["name"] == version_name), None
                )
                if not version:
                    version = require(
                        client.post(
                            "/rest/api/3/version",
                            json={
                                "projectId": int(detail["id"]),
                                "name": version_name,
                                "description": "Evidence-backed outcome release",
                                "releaseDate": "2026-09-30",
                                "released": False,
                            },
                        ),
                        f"Create release {key}",
                    )
                ...
                if key in LEGACY_KEYS and len(existing) >= 6:
                    story_keys = [
                        ...
                        and row["fields"].get("issuetype", {}).get("name", "").lower()
                        != "epic"
                    ][:6]
                else:
                    for _index, (base_summary, points, priority) in enumerate(
                        STORIES, start=1
                    ):
                        ...
                        if not issue:
                            issue_fields = {
                                ...
                                "summary": summary,
                                "description": adf(
                                    f"Real Jira delivery evidence for {project['name']}."
                                ),
                                "labels": [
                            ...
                            issue = require(
                                client.post(
                                    "/rest/api/3/issue", json={"fields": issue_fields}
                                ),
                    ...
                    if not already:
                        require(
                            client.post(
                                ...
                                json={
                                    "type": {"name": blocks_type},
                                    "outwardIssue": {"key": story_keys[4]},
                                    "inwardIssue": {"key": story_keys[5]},
                                },
                            ),
                            f"Create blocker {key}",
                        )
                        counts["links_created"] += 1
                counts["projects"] += 1
                print(f"KPI evidence ready: {key} ({len(story_keys)} stories)")
        print(
            f"Jira KPI evidence completed at {datetime.now(UTC).isoformat()}: {counts}"
        )
    finally:
        db.close()
