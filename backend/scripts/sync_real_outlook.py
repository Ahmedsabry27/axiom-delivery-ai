...
def graph_get(client: httpx.Client, url: str, params: dict | None = None) -> dict:
    ...
    if response.status_code != 200:
        raise RuntimeError(
            f"Microsoft Graph read failed with HTTP {response.status_code}"
        )
...
def paged(
    client: httpx.Client, url: str, params: dict | None = None, limit: int = 1000
) -> list[dict]:
    rows: list[dict] = []
...
def main() -> None:
    ...
    try:
        row = (
            ...
            .filter_by(tenant_id="axiom-demo", connector_type="outlook_calendar")
            .one()
        )
                ...
                imported.append(
                    {
                        ...
                        "version": str(calendar.get("changeKey") or "1"),
                        "updated": now,
                        "url": None,
                        "payload": {
                            "name": calendar.get("name"),
                            "can_edit": calendar.get("canEdit"),
            ...
            imported.append(
                {
                    ...
                    "updated": parse_time(
                        event.get("lastModifiedDateTime") or start_value, now
                    ),
        ...
        for item in imported:
            encoded = json.dumps(
                item["payload"], sort_keys=True, separators=(",", ":"), default=str
            )
            ...
            record = (
                ...
                .filter_by(
                    tenant_id=row.tenant_id,
                    provider_tenant_id=account_id,
                    external_entity_type=item["type"],
            ...
            if not record:
                record = IntegrationSourceRecord(
                    ...
                    external_entity_type=item["type"],
                    external_entity_id=item["id"],
                    canonical_entity_type=item["canonical"],
                ...
                or record.source_version != item["version"]
            ):
                record.source_version = item["version"]
                record.source_updated_at = item["updated"]
                record.content_fingerprint = fingerprint
                record.title = item["title"]
                record.source_url = item["url"]
                record.safe_payload = item["payload"]
                updated += 1
            else:
                unchanged += 1
        ...
        row.configuration = {
            **(row.configuration or {}),
            "simulator": False,
            "sync_cursor": cursor,
            "source_scope": {"selection": "All calendars authorized for Ahmed Sabry"},
            "sync_policy": {
                "mode": "INCREMENTAL",
                "window_past_days": 30,
                "window_future_days": 90,
                "bounded_record_limit": 1000,
            },
        }
        row.safe_metadata = {
        ...
        db.commit()
        print(
            f"Live Outlook synchronization succeeded: {len(calendars)} calendars and {len(events)} events read."
        )
    except Exception:
        db.rollback()
        raise
